...
dbc_file_path = sys.argv[2]  # Send the selected file path from the Qt GUI as an input argument
print(f"File Path: {dbc_file_path}");
file_name = os.path.splitext(os.path.basename(dbc_file_path))[0] # Extract the file name to use for the JSON
json_file_path = working_dir + f'/JSON/{file_name}.json'  # Default JSON output folder for HeavyInsight
